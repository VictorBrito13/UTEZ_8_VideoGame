...
def populate_base_creatures():
    ...
    while count < 100:
        try:
            ...
            species_resp = requests.get(f"https://pokeapi.co/api/v2/pokemon-species/{current_id}")
            ...
            pokemon_resp = requests.get(f"https://pokeapi.co/api/v2/pokemon/{current_id}")
            ...
            t2_name = data['types'][1]['type']['name'] if len(data['types']) > 1 else None
