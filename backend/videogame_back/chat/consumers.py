...
class ChatConsumer(AsyncWebsocketConsumer):
  ...
  async def receive(self, text_data):
    ...
    # TODO: change to scope['user'] for production
  ...
  def save_message(self, user_id, message_text):
    # TODO: Tech Lead - Ensure proper error handling for database constraints
    try:
      battle = Battle.objects.get(id=self.battle_id)
      user = User.objects.get(id=user_id)
      ChatMessage.objects.create(
        battle=battle, sender=user, message=message_text
      )
